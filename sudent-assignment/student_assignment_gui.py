...
import student_assignment_main
import student_assignment_func
...
def load_gui(self):
    ...
    self.txt_fname = tk.Entry(self.master,text='Test')
    ...
    self.lstList1.bind('<<ListboxSelect>>',lambda event: student_assignment_func.onSelect(self,event))
    ...
    self.btn_add = tk.Button(self.master,width=12,height=2,text='Add',command=lambda: student_assignment_func.addToList(self))
    self.btn_add.grid(row=10,column=0,padx=(25,0),pady=(45,10),sticky=W)
    self.btn_update = tk.Button(self.master,width=12,height=2,text='Update',command=lambda: student_assignment_func.onUpdate(self))
    self.btn_update.grid(row=10,column=1,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_delete = tk.Button(self.master,width=12,height=2,text='Delete',command=lambda: student_assignment_func.onDelete(self))
    self.btn_delete.grid(row=10,column=2,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_close = tk.Button(self.master,width=12,height=2,text='Close',command=lambda: student_assignment_func.ask_quit(self))
    self.btn_close.grid(row=10,column=4,columnspan=1,padx=(15,0),pady=(45,10),sticky=E)

    student_assignment_func.create_db(self)
    student_assignment_func.onRefresh(self)
