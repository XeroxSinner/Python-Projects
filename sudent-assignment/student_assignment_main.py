...
"""
    - DONE - Page labeled 'Student Tracking'
    - Form to include: First, Last, Ph, Em, 'Current Course', submit button
    - Listbox display w/ all relevant info above (? seems like a lot for a listbox but...)
    - delete button w/ funcitonality
"""
...
class ParentWindow(Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        student_assignment_func.center_window(self,500,400)
        self.master.title("Student Tracking")
        self.master.configure(bg="#F0F0F0")
        # This protocol method is a tkinter built-in method to catch if 
        # the user clicks the upper corner, "X" on Windows OS.
        self.master.protocol("WM_DELETE_WINDOW", lambda: student_assignment_func.ask_quit(self))
        arg = self.master

        # load in the GUI widgets from a separate module, 
        # keeping your code comparmentalized and clutter free
        student_assignment_gui.load_gui(self)
        ...
        filemenu.add_command(label="Exit", underline=1,accelerator="Ctrl+Q",command=lambda: student_assignment_func.ask_quit(self))
